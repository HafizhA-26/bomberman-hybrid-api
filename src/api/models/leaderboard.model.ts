...
export const LeaderboardModel = {
    ...
    async findByDeviceAndEnemy(req: LeaderboardRequest): Promise<LeaderboardData> {
        const { data: playerData, error: error_1 } = await supabase.from("leaderboards").select("players(username), action_count, playtime").match({ device_id: req.deviceId, enemy_type: req.enemyType }).maybeSingle();
    ...
    async upsert(req: LeaderboardRequest): Promise<LeaderboardData> {
        const { data: playerData, error: error_1 } = await supabase
            ...
            .upsert(
                ...
                {
                    onConflict: "device_id,enemy_type",
                },
            )
